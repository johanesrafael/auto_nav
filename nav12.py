...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb  3 13:09:49 2020
...
#    
    ...
    twist=Twist()
    a=0
    if np.amin(laser_range[0,front_angles])<=0.2 and np.amin(laser_range[0,front_angles])!=0.0:
        twist.linear.x=-linear_speed
        a=1
    else:
        twist.linear.x=linear_speed
...
#                if laser_range[0,i]!= 0 and laser_range[0,i] < minimum:
#                    minimum = laser_range[0,i]
#            
#            sideD = minimum
#            
#            minimum2 = 1000
#            for i in range(-3,4):
#                if laser_range[0,i]!=0 and laser_range[0,i] < minimum2:
#                    minimum2 = laser_range[0,i]
#                    
#            frontD = minimum2
            frontD=np.amin(laser_range[0,range(-3,4,1)])
            
            rospy.loginfo(str(sideD) +" "+str(frontD))
            
            if sideD<0.40 and sideD!= 0:
                if frontD<0.20 and frontD !=0:
                    twist.linear.x=0
                    time.sleep(0.1)
                    pub.publish(twist)
                    rotatebot(t_yaw-90)
                    ...
                    time.sleep(0.05)
                    pub.publish(twist)
                    if np.amin(laser_range[0,range(-3,4,1)])>0.40:
                        if np.amin(laser_range[0,range(87,94,1)])<0.18 and np.amin(laser_range[0,range(87,94,1)]) != 0:
                            rospy.loginfo(str(sideD)+" less than 0.17, turning out.")
                            rotatebotWhileMove(-12)
                            time.sleep(0.15)
                            if np.amin(laser_range[0,range(-3,4,1)])>0.30 and np.amin(laser_range[0,range(87,94,1)])<0.35 and np.amin(laser_range[0,range(87,94,1)])!= 0:
                                rospy.loginfo("Counter-TURN")
                                rotatebotWhileMove(9)
                                time.sleep(0.15)
                        elif np.amin(laser_range[0,range(87,94,1)])>0.23:
                            rospy.loginfo(str(sideD)+" more than 0.22, turning in.")
                            rotatebotWhileMove(12)
                            time.sleep(0.15)
                            if np.amin(laser_range[0,range(-3,4,1)])>0.30 and np.amin(laser_range[0,range(87,94,1)])<0.35 and np.amin(laser_range[0,range(87,94,1)]) != 0:
                                rospy.loginfo("Counter-TURN")
                ...
                time.sleep(0.1)
                pub.publish(twist)
                time.sleep((sideD/linear_speed)*0.4)
                twist.linear.x=0
                time.sleep(0.1)
                pub.publish(twist)
                time.sleep(0.1)
