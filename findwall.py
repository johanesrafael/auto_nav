...
#    
    ...
    twist=Twist()
    a=0
    if np.amin(laser_range[0,front_angles])<=0.40:
        twist.linear.x=-linear_speed
        a=1
    else:
        twist.linear.x=linear_speed
...
#            pub.publish(twist)
            ...
            if np.amin(laser_range[0,range(87,94,1)])<sideD+0.10:
                if np.amin(laser_range[0,range(-3,4,1)])<0.4 and np.amin(laser_range[0,range(-3,4,1)])!=0:
